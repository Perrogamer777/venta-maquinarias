...
def webp_to_jpg(webp_url: str) -> str:
    ...
    try:
        ...
        response = requests.get(webp_url, timeout=15)
        ...
        # Crear nombre único basado en hash de URL
        url_hash = hashlib.md5(webp_url.encode()).hexdigest()[:12]
        ...
        if blob.exists():
            logger.info(f"✅ Imagen ya convertida previamente: {jpg_filename}")
            blob.make_public()
